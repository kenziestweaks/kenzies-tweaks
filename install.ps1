...
public class KTButton : Button
{
    ...
    protected override void OnPaint(PaintEventArgs pevent)
    {
        ...
        if (_hovering && _variant != KTVariant.Ghost && _variant != KTVariant.Icon)
            bg = KT4.Blend(bg, Color.White, 0.10f);
...
function Test-UltimateUnlocked { try { return (powercfg /list 2>$null | Select-String 'e9a42b02-d5df-448d-aa00-03f14749eb61') -ne $null } catch { return $false } }
function Test-KenziePlanExists { try { return (powercfg /list 2>$null | Select-String 'b7701089-9d55-49a7-90ec-f6be3a3566a9') -ne $null } catch { return $false } }
...
function Test-GameModeEnabled {
    try { $v = Get-ItemProperty 'HKCU:\Software\Microsoft\GameBar' -Name 'AllowAutoGameMode' -ErrorAction SilentlyContinue; if ($v -and $v.AllowAutoGameMode -eq 1) { return $true } } catch { }
    return $false
}
function Test-GameBarDisabled {
    try { $v = Get-ItemProperty 'HKCU:\Software\Microsoft\Windows\CurrentVersion\GameDVR' -Name 'AppCaptureEnabled' -ErrorAction SilentlyContinue; if ($v -and $v.AppCaptureEnabled -eq 0) { return $true } } catch { }
    return $false
}
...
function New-TbBtn {
    ...
    $b.Cursor = "Hand"
    $hc = $HoverColor; $hb = $HoverBg
    $b.Add_MouseEnter({ param($s, $e) $s.ForeColor = $hc; $s.BackColor = $hb })
    $b.Add_MouseLeave({ param($s, $e) $s.ForeColor = $D.TextMid; $s.BackColor = [System.Drawing.Color]::Transparent })
    return $b
}
...
function Show-Toast {
    ...
    $toast.Add_Paint({
        ...
        $pen = [System.Drawing.Pen]::new($capturedC.Br, [float]1.0)
        $g.DrawPath($pen, $path)
        $pen.Dispose()
        $path.Dispose()
    })
...
$aboutVer.Text = "Version 4.2  -  Fixed"
...
function Update-ActivePlanDisplay {
    ...
    if ($ag) {
        ...
        foreach ($key in $script:PlanButtons.Keys) {
            if ($key -eq $ag) { $script:PlanButtons[$key].Variant = "Primary" }
            else { $script:PlanButtons[$key].Variant = "Secondary" }
