...
<#
    Kenzie's Tweaks v4.1 - Full UI/UX Rebuild (syntax-validated)
#>
...
function New-Pen {
    ...
    $p = [System.Drawing.Pen]::new($Color, [float]$Width)
    return $p
}
...
$brandTitle.AutoSize = $true$brandTitle.BackColor = [System.Drawing.Color]::Transparent
...
function Show-Modal {
    ...
    if ($script:ActiveModal) { try { $script:ActiveModal.Backdrop.Dispose() } catch { } }
...
$sideToggle.Add_Click({
    Set-SidebarState -Expanded (-not $script:SidebarState_Expanded)
})
...
$aboutVer.Text = "Version 4.1  -  Full UI/UX Rebuild"
